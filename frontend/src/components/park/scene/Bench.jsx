...
// The user "sits" when clicking (or triggering XR pointer) on the bench.
  ...
  return (
    ...
    >
      ...
      {[-0.2, 0, 0.2].map((zOff, i) => (
        <mesh
          key={`seat-${i}`}
          position={[0, seatHeight, zOff * seatDepth]}
          castShadow
          receiveShadow
        >
          <boxGeometry args={[seatWidth, 0.05, 0.14]} />
          <meshStandardMaterial
            color={i === 1 ? woodLight : woodDark}
            roughness={0.7}
            metalness={0.05}
            emissive={emissive}
      ...
      {[0, 0.22, 0.44].map((yOff, i) => (
        <mesh
          key={`back-${i}`}
          position={[0, seatHeight + 0.15 + yOff, -seatDepth / 2 - 0.02]}
          rotation={[-0.12, 0, 0]}
          castShadow
          receiveShadow
        >
      ...
      {[-1, 1].map((side) => (
        <mesh
          key={`arm-${side}`}
          position={[side * (seatWidth / 2 - 0.02), seatHeight + 0.25, 0]}
          castShadow
        >
      ...
      {/* Front & rear legs (metal-like frame) */}
      {[
      ...
      {/* Ground shadow disc (tiny) - implicit via directional light shadow */}

      {/* Invisible highlight ring for hover */}
      {highlighted && (
